...
public class RaumStrukturTest
{
	...
	public void setUp() throws Exception
	{
		...
		xmlraumA= new XmlRaum(raumA.getId(), 0,raumA.getAusgang("ost").getId(),
				0,raumA.getAusgang("west").getId(), raumA.getX(), raumA.getY(), raumA.getItems());
		
		xmlraumB= new XmlRaum(raumB.getId(), raumB.getAusgang("nord").getId(),0,
				0,raumB.getAusgang("west").getId(), raumB.getX(), raumB.getY(), raumB.getItems());
		
		xmlraumC= new XmlRaum(raumC.getId(),0,raumC.getAusgang("ost").getId(),
				0,0, raumC.getX(), raumC.getY(), raumC.getItems());
		
		xmlraumD= new XmlRaum(raumD.getId(), 0,0,
				raumD.getAusgang("süd").getId(),0, raumD.getX(), raumD.getY(), raumD.getItems());
	...
	{
		List<XmlRaum> xmlliste = new ArrayList<XmlRaum>();
		xmlliste=raumstruktur.getXMLRaumListe();
